unit VTPInterface_TCPIP_nothread;
...
uses
  ...
  HWAbstractDevicesNew2, MyAquireThreadPrototype,  sockets, MyTCPClient_old;
 ...
 TVTPDeviceReg = class ( TVTPDeviceBase )
  ...
  TMyAquireThread = class (TObject)       //TMultiReadExclusiveWriteSynchronizer.
    ...
    private
      ...
      function ProcSingleReply( str: string; devstr: string): boolean;
      //
...
//    property IsReady: boolean read fReady;
     private
      ...
      function getLastElapsedMS: longint; override;
  ...
  srcname:= TStringlist.Create;
    ...
    if fDebug then LeaveLogMsg( ' got enabled devices count:  ' + IntToStr(nen));
     ...
     for i:=0 to Length(toklist)-1 do
       begin
         if not ProcSingleReply( toklist[i].s, srcname.Strings[i] ) then begin
           Inc(err);
...
function TMyAquireThread.ProcSingleReply( str: string; devstr: string): boolean;
Var
...
begin
  ...
  if fDebug then LeaveLogMsg('  ProcSingleReply:  dev:' + devstr  +' toklist= ' + TokenListToStr(singletoklist) );
  if Length(singletoklist)<2 then begin
    if fDebug then LeaveLogMsg('  E: Length(singletoklist)<2');
    exit;
    end;
  if LowerCase( singletoklist[0].s )<>'ok' then begin
    if fDebug then LeaveLogMsg('  E: <>OK');
    exit;
    end;
  devidstr := devstr;
  valstr := singletoklist[1].s;
  //
...
begin
  inherited Create(CInterfaceVer, false);
...
begin
  ...
  locked := dsync.LockTimeout( CLockWaitMax );
     ...
     fLastAquireTimeMS := dsync.fLastAquireDurMS;
...
function TVTPControl_TCP_FCSControl.getLastElapsedMS: longint;
begin
  Result := fLastAquireTimeMS;
end;
