unit PTCInterface_KolPTC_DynamicDLL_new;
...
function TKolPTCObject.ReadFuseState(Var fuseHARD: boolean; Var fuseSoft: boolean): boolean;
Const
...
begin
  fuseHard := false;
  fuseASoft := false;
  Result := ReadRegProtectStatus( fuse );
  if Result then
    begin
      fuseHARD := BitIsSet(fuse, 1);
      fuseSoft := BitIsSet(fuse, 4);
    end;
  if fPTCDebug then logmsg('   TKolPTCObject.GetFuseState res=' + IntToStr(fuse) );
  //update flags!!!
