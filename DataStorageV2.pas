...
uses
  ...
  MyUtils, Logger, MyParseUtils, MyThreadUtils;
...
procedure DataStorageInit;       //call after app start
procedure DataStorageFinish;     //call before app closes to destroy objects
...
procedure DataStorageInit;       //call after app start
begin
   if MonitorFileMain=nil then  MonitorFileMain := TMonitorFileStorage.Create;
   if MonitorMemHistory=nil then  MonitorMemHistory := TMonitorMemDataStorage.Create;
   logfiledebug := false;
end;
...
procedure DataStorageFinish;     //call before app closes to destroy objects
begin
   if MonitorFileMain<>nil then  MonitorFileMain.Destroy;
   if MonitorMemHistory<>nil then MonitorMemHistory.Destroy;
end;
...
begin
  Result := 'Time_from_start[ms]' + #9 + 'Temp[degC]' + #9 + 'Current[A.cm-2]'
            + #9 + 'Voltage[V]' + #9 + 'Power[W.cm-2]' + #9 + 'FullTime' + #9 + 'Uref[V]' + #9 + 'debugmsg' + #9 + 'FlowData(sccm,bar)';
end;
...
begin
  Result := 'RelTime[s]' + #9 + 'Current[A.cm-2]'
            + #9 + 'Voltage[V]' + #9 + 'Power[W.cm-2]' + #9 + 'FullTime'
            + #9 + 'Uref[V]' + #9 + 'PTCsetpoint[A/V]'
