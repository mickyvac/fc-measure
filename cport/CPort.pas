...
const
  CBR_230400 = $38400;
  {$EXTERNALSYM CBR_230400}
  CBR_460800 = $70800;
  {$EXTERNALSYM CBR_460800}
  CBR_921600 = $E1000;
  {$EXTERNALSYM CBR_921600}
  CBR_1M = $F4240;
...
type
  ...
  TBaudRate = (brCustom, br110, br300, br600, br1200, br2400, br4800, br9600, br14400,
                br19200, br38400, br56000, br57600, br115200, br128000, br256000, br230400, br460800, br921600, br1mega);       //230400 460800 921600
  ...
  protected
    ...
    procedure StoreIniFile(Var IniFile: TIniFile); virtual;
    procedure LoadRegistry(Reg: TRegistry); virtual;
    procedure LoadIniFile(Var IniFile: TIniFile); virtual;
  ...
  public
    ...
    procedure PurgeCommTotal;  //added by Michal Vaclavu 2016-06-01    
...
begin
  if Connected then Close;        //!!!DAMN there was need to add the test for "connected"!! o
                                  //therwise it would do something
                                  //unknown and causes thread to HANG when TERMINATING
                                  //added 2016-05-19 Mihcal Vaclavu
  FBuffer.Free;
  FFlowControl.Free;
  FTimeouts.Free;
  FParity.Free;
...
const
  ...
  CBaudRate: array[TBaudRate] of Integer =
    (0, CBR_110, CBR_300, CBR_600, CBR_1200, CBR_2400, CBR_4800, CBR_9600,
     ...
     CBR_128000, CBR_256000, CBR_230400, CBR_460800, CBR_921600, CBR_1M);
...
procedure TCustomComPort.PurgeCommTotal;
//Added 2016-06-01 by Michal Vaclavu
var
  Flag: DWORD;
begin
  Flag := PURGE_RXCLEAR or PURGE_TXCLEAR or PURGE_RXABORT or PURGE_TXABORT;
  if not PurgeComm(FHandle, Flag) then
    //raise EComPort.Create
    CallException(CError_PurgeFailed, GetLastError);
end;
...
procedure TCustomComPort.StoreIniFile(Var IniFile: TIniFile);
Var
  name: string;
begin
  Name := 'comport';
...
procedure TCustomComPort.LoadIniFile(Var IniFile: TIniFile);
Var
  name: string;
begin
  Name := 'comport';
...
const
  BaudRateStrings: array[TBaudRate] of string = ('Custom', '110', '300', '600',
    ...
    '115200', '128000', '256000', '230400', '460800', '921600', '1Mbps');
begin
