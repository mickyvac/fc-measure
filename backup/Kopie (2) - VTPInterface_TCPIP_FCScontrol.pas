...
uses
  ...
  HWAbstractDevicesNew, MyAquireThreadProtoype,  sockets;
...
Type
  ...
  public
    ...
    destructor Destroy; override;
  public
    // this arrays of device parameters (a COPY from similar array in the main object) will be used by aquire thread to poll each added device (read only)
    //only devices that are marked as enabled will polled for data, device Idstr are already in the records, and are assigned in the main object
    ValveParams: array[TValveDevices] of TValveParamRec;
    TempDevParams: array [TTempDevices] of TTempDeviceParamRec;
    PressSensParams: array [TPressureSensDevices] of TPressureSensParamRec;
    PressRegParams: array [TPressureRegDevices] of TPressureRegParamRec;
    OtherDevParams: array [TOtherDevices] of TValveParamRec;    //for now, it is for sw switches, which are the same as valve switches
...
{    procedure AddDev(dev: TTempDevices); overload;
  ...
  TMyAquireThread = class (TThread)       //TMultiReadExclusiveWriteSynchronizer.
    ...
    private
      //dev works as ITERATOR - at beginning set it to Low(TXXXXDevices) and after each call it will be incremented
      //data are fetched from arrays in datasynchro
      //only devices, that are ENABLED are returned, if there are NO MORE enabled devices, result is FALSE
      function getNextDev(Var dev: TValveDevices; Var idstr: string): boolean; overload; //dev works as ITERATOR
      function getNextDev(Var dev: TTempDevices; Var idstr: string): boolean; overload;  //dev works as ITERATOR
      function getNextDev(Var dev: TPressureSensDevices; Var idstr: string): boolean; overload;  //dev works as ITERATOR
      function getNextDev(Var dev: TPressureRegDevices; Var idstr: string): boolean; overload; //dev works as ITERATOR
      function getNextDev(Var dev: TOtherDevices; Var idstr: string): boolean; overload;  //dev works as ITERATOR
      //
    ...
    private
      ...
      function THDCheckAssignComClient(Var client: TTCPClient; fromstr: string): boolean;
...
//    property IsReady: boolean read fReady;
    ...
    public
      ...
      function CheckAssignComClient(Var client: TTCPClient; fromstr: string): boolean;
...
begin
  ...
  while not Terminated do
    ...
    cnt := 0;
    vdev := Low( TValveDevices );
    while getNextDev(vdev, idstr) do
      begin
    ...
    tdev := Low( TTempDevices);
    while getNextDev(tdev, idstr) do
      begin
        if not comSend('GET '+idstr,  replystr, CTCPTimeoutMS) then LeaveLogMsg('EEE com error when processing dev ' + idstr);
        procDev( tdev, idstr, replystr);
        Inc(cnt);
      end;
    psdev := Low( TPressureSensDevices);
    while getNextDev(psdev, idstr) do
      begin
        if not comSend('GET '+idstr,  replystr, CTCPTimeoutMS) then LeaveLogMsg('EEE com error when processing dev ' + idstr);
        procDev( psdev, idstr, replystr);
        Inc(cnt);
      end;
    prdev := Low( TPressureRegDevices);
    while getNextDev(prdev, idstr) do
      begin
        if not comSend('GET '+idstr,  replystr, CTCPTimeoutMS) then LeaveLogMsg('EEE com error when processing dev ' + idstr);
        procDev( prdev, idstr, replystr);
        Inc(cnt);
      end;
    odev := Low( TOtherDevices);
    while getNextDev(odev, idstr) do
      begin
        if not comSend('GET '+idstr,  replystr, CTCPTimeoutMS) then LeaveLogMsg('EEE com error when processing dev ' + idstr);
    ...
    //
    if fDebug or true then LeaveLogMsg('execute - iterated devices: ' + IntToStr(cnt));
...
function TMyAquireThread.getNextDev(Var dev: TValveDevices; Var idstr: string): boolean;  //dev works as ITERATOR
//dev works as ITERATOR - at beginning set it to Low(TXXXXDevices) and after each call it will be incremented
//data are fetched from arrays in datasynchro
//only devices, that are ENABLED are returned, if there are NO MORE enabled devices, result is FALSE
begin
  ...
  DevicesSynchro.BeginRead;
    while true do
      begin
        if DevicesSynchro.ValveParams[ dev ].enabled then
          begin
            idstr := DevicesSynchro.ValveParams[ dev ].idstr;
            Result := true;
            break;
          end;
        if dev=High(TValveDevices) then break;
        Inc( dev );
      end;
  DevicesSynchro.EndRead;
end;

function TMyAquireThread.getNextDev(Var dev: TTempDevices; Var idstr: string): boolean;   //dev works as ITERATOR
//dev works as ITERATOR - at beginning set it to Low(TXXXXDevices) and after each call it will be incremented
//data are fetched from arrays in datasynchro
//only devices, that are ENABLED are returned, if there are NO MORE enabled devices, result is FALSE
begin
  ...
  DevicesSynchro.BeginRead;
    while true do
      begin
        if DevicesSynchro.TempDevParams[ dev ].enabled then
          begin
            idstr := DevicesSynchro.TempDevParams[ dev ].idstr;
            Result := true;
            break;
          end;
        if dev=High(TTempDevices) then break;
        Inc( dev );
      end;
  DevicesSynchro.EndRead;
end;

function TMyAquireThread.getNextDev(Var dev: TPressureSensDevices; Var idstr: string): boolean;   //dev works as ITERATOR
//dev works as ITERATOR - at beginning set it to Low(TXXXXDevices) and after each call it will be incremented
//data are fetched from arrays in datasynchro
//only devices, that are ENABLED are returned, if there are NO MORE enabled devices, result is FALSE
begin
  ...
  DevicesSynchro.BeginRead;
    while true do
      begin
        if DevicesSynchro.PressSensParams[ dev ].enabled then
          begin
            idstr := DevicesSynchro.PressSensParams[ dev ].idstr;
            Result := true;
            break;
          end;
        if dev=High(TPressureSensDevices) then break;
        Inc( dev );
      end;
  DevicesSynchro.EndRead;
end;

function TMyAquireThread.getNextDev(Var dev: TPressureRegDevices; Var idstr: string): boolean;  //dev works as ITERATOR
//dev works as ITERATOR - at beginning set it to Low(TXXXXDevices) and after each call it will be incremented
//data are fetched from arrays in datasynchro
//only devices, that are ENABLED are returned, if there are NO MORE enabled devices, result is FALSE
begin
  ...
  DevicesSynchro.BeginRead;
    while true do
      begin
        if DevicesSynchro.PressRegParams[ dev ].enabled then
          begin
            idstr := DevicesSynchro.PressRegParams[ dev ].idstr;
            Result := true;
            break;
          end;
        if dev=High(TPressureRegDevices) then break;
        Inc( dev );
      end;
  DevicesSynchro.EndRead;
end;
...
function TMyAquireThread.getNextDev(Var dev: TOtherDevices; Var idstr: string): boolean;   //dev works as ITERATOR
//dev works as ITERATOR - at beginning set it to Low(TXXXXDevices) and after each call it will be incremented
//data are fetched from arrays in datasynchro
//only devices, that are ENABLED are returned, if there are NO MORE enabled devices, result is FALSE
begin
  ...
  DevicesSynchro.BeginRead;
    while true do
      begin
        if DevicesSynchro.OtherDevParams[ dev ].enabled then
          begin
            idstr := DevicesSynchro.OtherDevParams[ dev ].idstr;
            Result := true;
            break;
          end;
        if dev=High(TOtherDevices) then break;
        Inc( dev );
      end;
  DevicesSynchro.EndRead;
end;
...
begin
  ...
  if cmd.t = CVTPCmdUser then
    begin
      ...
      LeaveLogMsg('USER CMD reply: "' + BinStrToPrintStr(reply) + '"' );
      datasynchro.BeginWrite;
...
Const
  CThisMod = 'TMyAquireThread.comSend';
begin
  Result := false;
  if not THDCheckAssignComClient(com, CThisMod) then exit;
  ...
  if Com.Connected then
    begin
     e := true;
     if com.WaitForData(0) then                       //WSAAsyncSelect(), WSAAsyncEvent(), or select().
      begin
        LeaveLogMsg('WaitForData before send TRUE: ');
        com.ReceiveBuf(buf, 100); //clear input buffer 
      end
      else
        LeaveLogMsg('WaitForData before send FALSE: ');
     n := Com.Sendln(cmd);  //terminating string is <CR> only         //TClientSocket  //TTCPClient
      LeaveLogMsg('comSend: cmdlen ='+ IntToStr( Length(cmd) )  +' SendLn n=' + IntToStr(n) );
    end
  else
    begin
      e := false;
...
begin
  if not THDCheckAssignComClient(com, CThisMod) then exit;
  if fDebug or true then  LeaveLogMsg('TTT ComRxChar: n=' + IntToStr( DataLen));
  //synchro
  s := '';
  marklock := false;
  ...
  //store
  if fDebug then  LeaveLogMsg('received: "' + BinStrToPrintStr( s ) + '"');
  rxbuf := rxbuf + s;
...
begin
  ...
  while d>Now() do
   begin
    ...
    len := 0;
    client.PeekBuf(buf, len);
    if len>0 then LeaveLogMsg('waiting N in buf: '+ IntToStr( len ) );
    s := '';
    if len>0 then
     begin
       for i := 0 to len do s := s + buf[i];
       LeaveLogMsg('dump buf: '+ s );
     end;

    if client.WaitForData(50) then                       //WSAAsyncSelect(), WSAAsyncEvent(), or select().
      begin
        LeaveLogMsg('Wait for data TRUE: ');
        //s := client.Receiveln( #11 );
        len := 0;
        n := client.ReceiveBuf(ax, 3);
        s := '';
        for i:=0 to 49 do begin s:= s + chr( ax[i] ); ax[i] := 0; end;
        LeaveLogMsg('recbuf dump buf: len '+ IntToStr(len) + ' str=' + BinStrToPrintStr(s) );
      end;
  comsynchro.EndWrite;
  fSynchroLockedByWait := false;
...
function TMyAquireThread.THDCheckAssignComClient(Var client: TTCPClient; fromstr: string): boolean;
//helper: if not il returns true including comsynchro.TCPClient
begin
  Result := false;
  client := nil;
  if comsynchro=nil then exit;
  ...
  if client=nil then
    begin
      LogError(fromstr + ': TCPClient=nil');
      exit;
    end;
  Result := true;
end;
...
function TVTPControl_TCP_FCSControl.CheckAssignComClient(Var client: TTCPClient; fromstr: string): boolean;
//helper: if not il returns true including comsynchro.TCPClient
begin
  Result := false;
  client := nil;
...
begin
  inherited;
  //disable all devices
  for vdev := Low(TValveDevices) to High(TValveDevices) do ValveParams[vdev].enabled := false;
  for tdev := Low(TTempDevices) to High(TTempDevices) do TempDevParams[tdev].enabled := false;
  for psdev := Low(TPressureSensDevices) to High(TPressureSensDevices) do PressSensParams[psdev].enabled := false;
  for prdev := Low(TPressureRegDevices) to High(TPressureRegDevices) do PressRegParams[prdev].enabled := false;
  for odev := Low(TOtherDevices) to High(TOtherDevices) do OtherDevParams[odev].enabled := false;
end;
