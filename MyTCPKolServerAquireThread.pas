unit MyTCPKolServerAquireThread;
...
uses
  ...
  HWAbstractDevicesV3, MyAquireThreadNEW_TCPIP, MyTCPClientForKolServer,
...
Type
  ...
  TAquireThread_KolServer_TCPIP = class (TObject)
    ...
    public
      procedure setTCPconf( server: string; port: string; ProtocolVers: integer);    //called from main thread, must not block
      procedure getTCPconf( Var server: string; Var port: string; Var ProtocolVers: integer ); //called from main thread, must not block
    ...
    private
      fDebug: boolean;
      fExternalFormatSettings: TFormatSettings;
      procedure setDebug(b: boolean);
    public
      property Debug: boolean read fDebug write setDebug;
      property FormatSet: TFormatSettings read fExternalFormatSettings;
    private
      //helper
...
Var
  IdKSdoingInit: string = '_DoingInit';
  IdKSLastAquireTS: string = '_LastAquireTimeStamp';
  IdKSLastElapsedMS: string = '_LastAquireElapsedMS';
  IdKSAnswerLowLevel: string = '_AnswerLowLevel';       //for use with user cmd - answer to last USER cmd - in raw as received
  IdKSCLientState: string = '_CLientState';
  IdKSCLientStatusStr: string =  '_CLientStatusStr';
  IdKSInterfaceReady: string = '_InterfaceReady';
  IdKSInitScriptEnabled: string = 'IdKSInitScriptEnabled';
  IdKSInitScriptCmd: string = 'IdKSInitScriptCmd';
...
begin
  ...
  fLastAquireTS := fDataRegistry.GetOrCreateItem( IdKSLastAquireTS );
  fLastElapsedMS := fDataRegistry.GetOrCreateItem( IdKSLastElapsedMS );
  fDataRegistry.GetOrCreateItem( IdKSCLientState );
  fDataRegistry.GetOrCreateItem( IdKSCLientStatusStr );
  fDataRegistry.GetOrCreateItem( IdKSInterfaceReady );
  ...
  //
  GetLocaleFormatSettings(0, fExternalFormatSettings);
  with fExternalFormatSettings do
  begin
    DecimalSeparator := '.';
    ThousandSeparator := ',';
  end;
...
begin
  ...
  if fDoingInit.valBool and (not fready.valBool) then
    begin
      conOK := false;
      if not fTCPclient.IsReady then fTCPClient.Open;
...
begin
   ...
   genAcquireList;    //verify and update faquireNameslist
    ...
    s := fAcquireNamesList.Text;
    if n>0 then
      begin
         s := '';
         if fDebug then fLogMsg('  querylist->' + fAcquireNamesList.Text );
         b := fTCPclient.QueryGetVariables(fAcquireNamesList, fReplylist);

         if fDebug then fLogMsg('  reply: '+ BoolToStr(b)   + ' list->' + fReplylist.Text );
         if b then
           begin
             for i:=0 to n-1 do
               begin
...
procedure TAquireThread_KolServer_TCPIP.RunAfterInit;
Var
    RIiniscriptenabled, RIiniscript: TRegistryItem;
    sec: string;
begin
  if not fReady.valbool then exit;
  if fDebug then fLogmsg( 'RunAfterInit: ' + Data[ IdKSInitScriptEnabled ].valStr + ' cmd ' + Data[ IdKSInitScriptCmd ].valStr);
  if data[IdKSInitScriptEnabled].valBool then  fCommandQueue.Add( Data[ IdKSInitScriptCmd ].valStr );
end;
...
begin
 ...
 //fDataRegistry.valBool[ IdKSInterfaceReady ] := fready.valBool;
end;
...
begin
  ...
  fDataRegistry.valstr[ IdKSCLientState ] := ClientStateToStr( fTCPclient.ClientState );
  fDataRegistry.valstr[ IdKSCLientStatusStr ] := fTCPclient.ClientStateDescription;
end;




procedure TAquireThread_KolServer_TCPIP.setTCPconf( server: string; port: string; ProtocolVers: integer);    //called from main thread, must not block
begin
  if fAquireThread=nil then exit;
  fAquireThread.ConfigureTCP(server, port);
  fTCPclient.ProtocolVer := ProtocolVers;
end;

procedure TAquireThread_KolServer_TCPIP.getTCPConf( Var server: string; Var port: string; Var ProtocolVers: integer); //called from main thread, must not block
begin
  if fTCPclient=nil then exit;
  server := fTCPclient.ConfHost;
  port := fTCPclient.ConfPort;
  ProtocolVers := fTCPclient.ProtocolVer;
end;
