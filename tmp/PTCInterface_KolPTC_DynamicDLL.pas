...
{$R-}
  ...
  begin
    ...
    if GlobalConfig<>nil then if not GlobalConfig.initflag then kolerrormsg(CThisProcName + 'KolPTC not available, exiting');
    exit;
  end;
...
begin
    ...
    UpdateFlagInSet(kolstat.HWFuseActive, CPtcHardFuseActivated, fFlagSet);
    UpdateFlagInSet(kolstat.SoftLimActive, CPtcSoftLimitationActive, fFlagSet);
    //TODO: check flag indicators (overrange)
    //U, I overrange
    UpdateFlagInSet( (Ufin < fRngVoltRec.low) or (Ufin > fRngVoltRec.high) , CPtcOverRangeVoltage, fFlagSet );
    UpdateFlagInSet( (Ifin < fRngCurrRec.low) or (Ifin > fRngCurrRec.high) , CPtcOverRangeCurrent, fFlagSet );
     //
    with Status do
      begin
       flagSet := fFlagSet;
       ...
       //                                                          //set
       debuglogmsg := 'Output=' + BoolToStr(isLoadConnected) + '|HWFuse=' + BoolToStr(CPtcHardFuseActivated in flagSet) +
...
begin
  ...
  for c:=1 to fRetryCount do
    begin
      //
      b := Ptc_ReadStatus(@st.Setpoint, @st.Range, @st.Feedback, @st.OutputOn, @st.HWFuseActive);
      //
      Inc(fCommCntTotal);
      if b then break;
      Inc(fCommErrCorrectedCnt);
  ...
  Unlock;
 //FUSE STATUS READ DIRECTLY FROM REGISTERS!!!
 //must not be locked now - read reg checks lock
  b2 := ReadRegProtectStatus(prot);
  if not b2 then logmsg(CThisProcName + ' PROT STAT ... failed ' );
  if b2 then
    begin
      st.HWFuseActive := ((prot and $01) = 0) or ((prot and $08)<>0);
      st.SoftLimActive := (prot and $04) <> 0;
    end;
...
begin
  ...
  //update flags!!!
  UpdateFlagInSet(fuse, CPtcHardFuseActivated, fFlagSet );
  //finish
  Unlock;
  Result := b;
end;
...
begin
  ...
  if fPTCdebug then kolmsg( '   > ' + procident + 'result='+ BoolToStr(b) + ' len=' + IntToStr( retlen) + ' reply=' + BinStrToPrintStr( kolbuftostr( buf ) ) );    
  if not b then
    begin
      kolerrormsg(procident + ': failed!');
      Inc(fCommErrNotCorrCnt);
