...
Uses  MyTCPClient_indy, myutils, myparseutils, myThreadUtils, ConfigManager, //no more MyTCPClient
...
TMyTCPClientForKolServer = class (TMyTCPClientThreadSafe)
  ...
  public
      function QueryCmdReliable(cmd: string; Var reply: string; tottimeout: longint; lockcomm: boolean = true): boolean;
      ...
      function QueryDoCommands(Var ListIN: TStringList; Var ListOUT: TStringList; tottimeout: longint = -2): boolean;
         //uses internally "QueryCmdReliable"
         //   ListIN constains list of fuel commands as per XCT protocol
         //   ListOUT contains responses in the same order    example of one cmd:   'set ptc.Setpoint 0.04'  response 'ERROR 2'
  ...
  private
      ...
      fMinReconnectDelayMS: longword;
      fReconenctTryCount: longword;
  ...
  public
      ...
      property ReconnectEventCount: longword read fReconenctTryCount write fReconenctTryCount;
  private
      fComLock: TMyLockableObject; //protection aghainst reentrance into comm functions!!!
  private
...
begin
  Close;
  fComLock.Lock;   //force wait for any task to finish
...
begin
  if (TimeDeltaNowMS(fTCPLastTRYConnectDateTime) > fMinReconnectDelayMS) then
    begin
      Inc(fReconenctTryCount);
...
begin
  ...
  while true do
    begin
      pn := posex(CEVENTstringID, buf,1);
      if pn=0 then break;
      pn := pn + length( CEVENTstringID );  //set pos after "EVENT" word - begining of event msg
...
begin
  while true do
    begin
      ...
      pn := pn + length( CEVENTstringID );  //set start pos after "EVENT" word - begining of event msg
		  en := posex( fTerminatingStr, buf, pn);
		  //extract one EVENT
		  xn := en+length(fTerminatingStr);
		  count := xn-pn;
...
begin
  Result := false;
  if fComLock.IsLocked then exit;
  fComLock.Lock;
  MarkCmd('echo', echostr, idcmdstr);
  ...
  if b then
    begin
      ...
      if (i1>0) and (i2>0) then Result := true
        ...
        begin
          xLogmsg(procident + ' fProtocolVer:'+ IntToStr(fProtocolVer) +' fail verify reply res i1, i2:'+ IntToStr( i1 ) + ', '+ IntToStr( i2 ) );
          xLogerror( procident + ' fProtocolVer:'+ IntToStr(fProtocolVer) +' fail verify reply - "' + reply  + '" res i1, i2:'+ IntToStr( i1 ) + ', '+ IntToStr( i2 ) );
        end;
    end;
...
function TMyTCPClientForKolServer.QueryCmdReliable(cmd: string; Var reply: string; tottimeout: longint; lockcomm: boolean = true): boolean;
...
begin
  ...
  reply := '';
  //retflags := [];
  if lockcomm then fComLock.Lock;
  if not IsReady then
    begin
      ...
      if not IsReady then
        begin
          ...
          if lockcomm then fComLock.UnLock;
          exit;
        end;
    end;
...
begin
  Result := false;
  if (ListIN=nil) or (listOUT=nil) then exit;
  if fComLock.IsLocked then exit;
...
fComLock.Lock;
  ...
  bq := QueryCmdReliable( cmdstr, replystr, tottimeout, false );
  if not bq then
    begin
      fComLock.UnLock;
      exit;
...
function TMyTCPClientForKolServer.QueryDoCommands(Var ListIN: TStringList; Var ListOUT: TStringList; tottimeout: longint = -2): boolean;
Var
  cmdstr, replystr: string;
  i, ie: longint;
  bq, bp: boolean;
...
fComLock.Lock;
  ...
  for i:=0 to ie-1 do
    begin
      cmdstr := cmdstr + ListIN.Strings[i];
      if i<ie-1 then cmdstr := cmdstr + ';';
    end;
  //send query
  if tottimeout = -2 then tottimeout := fTimeoutSingle;
  ...
  // process reply in stringlist
  ParseStrSep(replystr, ';', tl);
...
begin
  ...
  Assert( inputlist.Count = length(toklist) );
  for i:=0 to length(toklist)-1 do
    begin
      SplitStrSimple(toklist[i].s, s1, s2);
      if length( s2 )>0 then
        begin
         if s1<>'OK' then sl.Add( inputlist[i] + '=' + toklist[i].s ); //must be ERROR - copy whole reply 
         sl.Add( inputlist[i] + '=' + s2 );  //OK copy only reply part of msg
        end
      else sl.Add( inputlist[i] + '=' + 'ERROR UNKNOWN1' );  //other non std situation
    end;
  Result := true;
end;
